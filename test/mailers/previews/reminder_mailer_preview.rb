...
class ReminderMailerPreview < ActionMailer::Preview
  ...
  def user
    User.find_by(github_username: 'gjtorikian')
  end

  def reminder_email
    ReminderMailer.with(user: user).reminder_email('gjtorikian@somewhere.com', 'gjtorikian', starred_repositories, has_public_repo_scope: false, frequency: :weekly)
  end

  def send_test_mail
    ReminderMailer.with(user: user).reminder_email(ENV['TEST_EMAIL_ADDRESS'], 'gjtorikian', starred_repositories, has_public_repo_scope: false, frequency: :weekly)
  end
end
